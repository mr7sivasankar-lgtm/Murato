...
export default function AdCard({ ad, onFavToggle, compact = false }) {
  ...
  return (
    ...
    >
      {/* Image */}
      {ad.images?.[0] ? (
        <img src={ad.images[0]} alt={ad.title} style={{ width: '100%', height: compact ? 90 : 105, objectFit: 'cover', display: 'block' }} />
      ) : (
        <div style={{ width: '100%', height: compact ? 90 : 105, background: 'linear-gradient(135deg,#e8edf5,#d1d9ef)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 44 }}>
          {catIcon}
          </div>
      )}
      ...
      <div style={{ position: 'absolute', top: 8, left: 8, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
        ...
        <span style={{ background: isService ? '#7c3aed' : '#1a2b5f', color: 'white', fontSize: 9, fontWeight: 700, padding: '3px 7px', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 3 }}>
      ...
      <div style={{ padding: '10px 10px 8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 5 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: '#6b7280', background: '#f3f4f6', padding: '2px 7px', borderRadius: 5 }}>
            {catIcon} {ad.category}
